...
Burdekin = read.csv("msPAF22_Total_Burdekin_Log.csv", header=T)

# > Clean df and create additional variables -----------
{
  Burdekin$DATE_conv <- as.Date(Burdekin$DATE, format="%d/%m/%Y",  tz = "Australia/Brisbane") # fix DATE format
  Burdekin$Month <- month(Burdekin$DATE_conv) # extract 'Month'
  Burdekin$Season <- ifelse(Burdekin$Month %in% c(11, 12, 1, 2, 3, 4), 'Wet', 'Dry') # create 'Season' variable (i applied Nov-April as standardised wet season)
  Burdekin$Season <- as.factor(Burdekin$Season)
  ...
  Burdekin$Month <- factor(Burdekin$Month, levels = financial_year_months)
  Burdekin$sqrtVALUE <- sqrt(Burdekin$Daily.Ave.PAF) # create sqrt of PAF
  str(Burdekin) # check
}
...
PAF_plot <- ggplot(Burdekin, aes(x = Month, y = Daily.Ave.PAF, fill=Season, alpha = 0.8)) +
  geom_boxplot() +
  ...
  theme_bw()
PAF_plot
ggsave("PAF plot by season and sampling year Burdekin.png", width = 15, height = 9, dpi = 1000)
...
x <- ggplot(data = Burdekin, aes(x =Daily.Ave.PAF)) + 
...
y <- ggplot(data = Burdekin, aes(x=Daily.Ave.PAF, fill=Season)) + # this line tells r which dataset to use (data =...) and which column to plot (x =...) 
...
z <- ggplot(Burdekin, aes(x = Month, y = Daily.Ave.PAF, fill = Season)) +
...
ggsave("PAF data visualisation Burdekin.png", width = 16, height = 9, dpi = 1000)


# > Investigate which transform to use on the data -------------
# plot log and sqrt transforms to see which one normalises the data better
fit.norm <- fitdist(Burdekin$logVALUE, "norm")
plot(fit.norm)

fit.norm <- fitdist(Burdekin$sqrtVALUE, "norm")
plot(fit.norm)
...
Dist_check_Burdekin <- descdist(Burdekin$sqrtVALUE, discrete = FALSE, boot = 1000)
Dist_check_Burdekin <- descdist(Burdekin$logVALUE, discrete = FALSE, boot = 1000)
#Burdekin use logvalue = 0,3 = normal distribution 
...
model = list(
  logVALUE ~ 1 + sin(time), # Using only sine for seasonality
  ~ 1 + time # Linear trend
)

ex = mcp_example("trigonometric")
fit = mcp(model, cores = 3, data = Burdekin)
...
write.csv(fit_summary, "Burdekin fit summary_JAGS.csv", row.names = FALSE)
...
# extract cp and find closest date for plotting
{
  ...
  print(cp_1) # view
  differences <- abs(Burdekin$time - cp_1) # calculate closest sqrtVALUE to cp_1
  closest_index <- which.min(differences) # find the index of this value 
  closest_date <-Burdekin$DATE[closest_index] # Get the corresponding DATE value that aligns with this index
...
BurdekinPlot <- plot(fit, q_predict = TRUE) + ggtitle("Change Point of Total Pesticides in Burdekin River at Home Hill Inkerman Bridge") + 
  labs(y= "log time-series PAF (%)", x = "Days since first sample
(05/10/2011 - 27/06/2023)") + 
  geom_vline(xintercept=cp_1, linetype='solid', col='red') +
  annotate("text", x=3000, y=4, label=closest_date, size=4, color="dark blue", angle = 90) +
  geom_hline(yintercept=PC95, linetype='dashed', col='blue') +
  annotate("text", x=4300, y=0.8989, label="PC95", size=4, color="dark blue") +
  geom_hline(yintercept=PC99, linetype='dashed', col='blue') +
  annotate("text", x=4300, y=0.2, label="PC99", size=4, color="dark blue") 
BurdekinPlot
ggsave("Burdekin CP with seasonality 11000.png", width = 16, height = 9, dpi = 1000)
