...
Mulgrave = read.csv("msPAF22_Total_Mulgrave_Log.csv", header=T)

# > Clean df and create additional variables -----------
{
  Mulgrave$DATE_conv <- as.Date(Mulgrave$DATE, format="%d/%m/%Y",  tz = "Australia/Brisbane") # fix DATE format
  Mulgrave$Month <- month(Mulgrave$DATE_conv) # extract 'Month'
  Mulgrave$Season <- ifelse(Mulgrave$Month %in% c(11, 12, 1, 2, 3, 4), 'Wet', 'Dry') # create 'Season' variable (i applied Nov-April as standardised wet season)
  Mulgrave$Season <- as.factor(Mulgrave$Season)
  ...
  Mulgrave$Month <- factor(Mulgrave$Month, levels = financial_year_months)
  Mulgrave$sqrtVALUE <- sqrt(Mulgrave$Daily.Ave.PAF) # create sqrt of PAF
  str(Mulgrave) # check
}
...
PAF_plot <- ggplot(Mulgrave, aes(x = Month, y = Daily.Ave.PAF, fill=Season, alpha = 0.8)) +
  geom_boxplot() +
...
x <- ggplot(data = Mulgrave, aes(x =Daily.Ave.PAF)) + 
...
y <- ggplot(data = Mulgrave, aes(x=Daily.Ave.PAF, fill=Season)) + # this line tells r which dataset to use (data =...) and which column to plot (x =...) 
...
z <- ggplot(Mulgrave, aes(x = Month, y = Daily.Ave.PAF, fill = Season)) +
...
fit.norm <- fitdist(Mulgrave$logVALUE, "norm")
plot(fit.norm)

fit.norm <- fitdist(Mulgrave$sqrtVALUE, "norm")
plot(fit.norm)
...
Dist_check_Mulgrave <- descdist(Mulgrave$sqrtVALUE, discrete = FALSE, boot = 1000)
Dist_check_Mulgrave <- descdist(Mulgrave$logVALUE, discrete = FALSE, boot = 1000)
...
fit = mcp(model, cores = 3, data = Mulgrave)
...
write.csv(fit_summary, "Mulgrave fit summary_JAGS.csv", row.names = FALSE)
...
# extract cp and find closest date for plotting
{
  ...
  print(cp_1) # view
  differences <- abs(Mulgrave$time - cp_1) # calculate closest sqrtVALUE to cp_1
  closest_index <- which.min(differences) # find the index of this value 
  closest_date <- Mulgrave$DATE[closest_index] # Get the corresponding DATE value that aligns with this index
...
MulgravePlot <- plot(fit, q_predict = TRUE) + ggtitle("Change Point of Total Pesticides in Mulgrave River at Deeral") + 
  labs(y= "log time-series PAF (%)", x = "Days since first sample
(25/11/2013 - 2023-06-02)") + 
  geom_vline(xintercept=cp_1, linetype='solid', col='red') +
  annotate("text", x=1323, y=4.5, label=closest_date, size=4, color="dark blue", angle = 90) +
  ...
  annotate("text", x=3500, y=1.1, label="PC99", size=4, color="dark blue") 
MulgravePlot
ggsave("Mulgrave CP with seasonality.png", width = 16, height = 9, dpi = 1000)
