...
fit = mcp(model, cores = 3, data = OConnell)
...
samples <- coda.samples(model, variable.names = c("alpha", "beta", "gamma", "sigma"), n.iter = 22000)
...
write.csv(fit_summary, "OConnell fit summary_JAGS.csv", row.names = FALSE)
...
# extract cp and find closest date for plotting
{
  ...
  print(cp_1) # view
  differences <- abs(OConnell$time - cp_1) # calculate closest sqrtVALUE to cp_1
  closest_index <- which.min(differences) # find the index of this value 
  closest_date <- OConnell$DATE[closest_index] # Get the corresponding DATE value that aligns with this index
...
OConnellPlot <- plot(fit, q_predict = TRUE) + ggtitle("Change Point of Total Pesticides in O'Connell River at Caravan Park") + 
  labs(y= "sqrt time-series PAF (%)", x = "Days since first sample
(09/08/2013 - 12/06/2023)") + 
  #geom_vline(xintercept=cp_1, linetype='solid', col='red') +
  #annotate("text", x=1550, y=8, label=closest_date, size=4, color="dark blue", angle = 90) +
  geom_hline(yintercept=PC95, linetype='dashed', col='blue') +
  annotate("text", x=3700, y=2.4, label="PC95", size=4, color="dark blue") +
  geom_hline(yintercept=PC99, linetype='dashed', col='blue') +
  annotate("text", x=3700, y=1.2, label="PC99", size=4, color="dark blue") 
OConnellPlot
ggsave("OConnell CP with seasonality.png", width = 16, height = 9, dpi = 1000)
