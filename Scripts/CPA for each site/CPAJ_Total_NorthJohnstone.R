...
Comet = read.csv("msPAF22_Total_NorthJohnstone_Log.csv", header=T)
...
PAF_plot <- ggplot(Comet, aes(x = Month, y = Daily.Ave.PAF, fill=Season, alpha = 0.8)) +
  ...
  theme_bw()
PAF_plot
ggsave("PAF plot by season and sampling year NorthJohnstone.png", width = 15, height = 9, dpi = 1000)
...
ggsave("PAF data visualisation NorthJohnstone.png", width = 16, height = 9, dpi = 1000)
...
#NorthJohnstone use logvalue = less skewness compared to sqrt 1,3
...
write.csv(fit_summary, "NorthJohnstone fit summary_JAGS.csv", row.names = FALSE)
...
CometPlot <- plot(fit, q_predict = TRUE) + ggtitle("Change Point of Total Pesticides in North Johnstone River at Goondi") + 
  labs(y= "log time-series PAF (%)", x = "Days since first sample
(29/02/2012 - 29/06/2023)") + 
  geom_vline(xintercept=cp_1, linetype='solid', col='red') +
  annotate("text", x=1520, y=3.8, label=closest_date, size=4, color="dark blue", angle = 90) +
  geom_hline(yintercept=PC95, linetype='dashed', col='blue') +
  annotate("text", x=4200, y=0.82, label="PC95", size=4, color="dark blue") +
  geom_hline(yintercept=PC99, linetype='dashed', col='blue') +
  annotate("text", x=4200, y=0.1, label="PC99", size=4, color="dark blue") 
CometPlot
ggsave("NorthJohnstone CP with seasonality.png", width = 16, height = 9, dpi = 1000)
