...
Comet = read.csv("msPAF22_Total_Herbert_Log.csv", header=T)
...
PAF_plot <- ggplot(Comet, aes(x = Month, y = Daily.Ave.PAF, fill=Season, alpha = 0.8)) +
  ...
  theme_bw()
PAF_plot
ggsave("PAF plot by season and sampling year Herbert.png", width = 15, height = 9, dpi = 1000)
...
ggsave("PAF data visualisation Herbert.png", width = 16, height = 9, dpi = 1000)
...
#Herbert use logvalue = less skewness compared to sqrt 1,3
...
write.csv(fit_summary, "Herbert fit summary_JAGS.csv", row.names = FALSE)
...
CometPlot <- plot(fit, q_predict = TRUE) + ggtitle("Change Point of Total Pesticides in Herbert River at John Row Bridge") + 
  labs(y= "log time-series PAF (%)", x = "Days since first sample
(14/7/2011 - 15/07/2023)") + 
  geom_vline(xintercept=cp_1, linetype='solid', col='red') +
  annotate("text", x=1690, y=3.8, label=closest_date, size=4, color="dark blue", angle = 90) +
  geom_hline(yintercept=PC95, linetype='dashed', col='blue') +
  annotate("text", x=4400, y=0.8989, label="PC95", size=4, color="dark blue") +
  geom_hline(yintercept=PC99, linetype='dashed', col='blue') +
  annotate("text", x=4400, y=0.2, label="PC99", size=4, color="dark blue") 
CometPlot
ggsave("Herbert CP with seasonality.png", width = 16, height = 9, dpi = 1000)
