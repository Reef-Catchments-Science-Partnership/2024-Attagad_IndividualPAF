...
Johnstone = read.csv("msPAF22_Total_Johnstone_Log.csv", header=T)

# > Clean df and create additional variables -----------
{
  Johnstone$DATE_conv <- as.Date(Johnstone$DATE, format="%d/%m/%Y",  tz = "Australia/Brisbane") # fix DATE format
  Johnstone$Month <- month(Johnstone$DATE_conv) # extract 'Month'
  Johnstone$Season <- ifelse(Johnstone$Month %in% c(11, 12, 1, 2, 3, 4), 'Wet', 'Dry') # create 'Season' variable (i applied Nov-April as standardised wet season)
  Johnstone$Season <- as.factor(Johnstone$Season)
  ...
  Johnstone$Month <- factor(Johnstone$Month, levels = financial_year_months)
  Johnstone$sqrtVALUE <- sqrt(Johnstone$Daily.Ave.PAF) # create sqrt of PAF
  str(Johnstone) # check
}
...
PAF_plot <- ggplot(Johnstone, aes(x = Month, y = Daily.Ave.PAF, fill=Season, alpha = 0.8)) +
  geom_boxplot() +
...
x <- ggplot(data = Johnstone, aes(x =Daily.Ave.PAF)) + 
...
y <- ggplot(data = Johnstone, aes(x=Daily.Ave.PAF, fill=Season)) + # this line tells r which dataset to use (data =...) and which column to plot (x =...) 
...
z <- ggplot(Johnstone, aes(x = Month, y = Daily.Ave.PAF, fill = Season)) +
...
fit.norm <- fitdist(Johnstone$logVALUE, "norm")
plot(fit.norm)

fit.norm <- fitdist(Johnstone$sqrtVALUE, "norm")
plot(fit.norm)
...
Dist_check_Johnstone <- descdist(Johnstone$sqrtVALUE, discrete = FALSE, boot = 1000)
Dist_check_Johnstone <- descdist(Johnstone$logVALUE, discrete = FALSE, boot = 1000)
...
fit = mcp(model, cores = 3, data = Johnstone)
...
write.csv(fit_summary, "Johnstone fit summary_JAGS.csv", row.names = FALSE)
...
# extract cp and find closest date for plotting
{
  ...
  print(cp_1) # view
  differences <- abs(Johnstone$time - cp_1) # calculate closest sqrtVALUE to cp_1
  closest_index <- which.min(differences) # find the index of this value 
  closest_date <- Johnstone$DATE[closest_index] # Get the corresponding DATE value that aligns with this index
...
JohnstonePlot <- plot(fit, q_predict = TRUE) + ggtitle("Change Point of Total Pesticides in Johnstone River at Coquette Point") + 
  labs(y= "log time-series PAF (%)", x = "Days since first sample
(20/10/2015 - 02/06/2023)") +
  geom_vline(xintercept=cp_1, linetype='solid', col='red') +
  annotate("text", x=100, y=4.5, label=closest_date, size=4, color="dark blue", angle = 90) +
  geom_hline(yintercept=PC95, linetype='dashed', col='blue') +
  annotate("text", x=2750, y=2.3, label="PC95", size=4, color="dark blue") +
  geom_hline(yintercept=PC99, linetype='dashed', col='blue') +
  annotate("text", x=2750, y=1.1, label="PC99", size=4, color="dark blue") 
JohnstonePlot
ggsave("Johnstone CP with seasonality.png", width = 16, height = 9, dpi = 1000)
