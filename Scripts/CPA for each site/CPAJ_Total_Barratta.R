...
Barratta = read.csv("msPAF22_Total_Barratta_Log.csv", header=T)

# > Clean df and create additional variables -----------
{
  Barratta$DATE_conv <- as.Date(Barratta$DATE, format="%d/%m/%Y",  tz = "Australia/Brisbane") # fix DATE format
  Barratta$Month <- month(Barratta$DATE_conv) # extract 'Month'
  Barratta$Season <- ifelse(Barratta$Month %in% c(11, 12, 1, 2, 3, 4), 'Wet', 'Dry') # create 'Season' variable (i applied Nov-April as standardised wet season)
  Barratta$Season <- as.factor(Barratta$Season)
  ...
  Barratta$Month <- factor(Barratta$Month, levels = financial_year_months)
  Barratta$sqrtVALUE <- sqrt(Barratta$Daily.Ave.PAF) # create sqrt of PAF
  str(Barratta) # check
}
...
PAF_plot <- ggplot(Barratta, aes(x = Month, y = Daily.Ave.PAF, fill=Season, alpha = 0.8)) +
  geom_boxplot() +
...
x <- ggplot(data = Barratta, aes(x =Daily.Ave.PAF)) + 
...
y <- ggplot(data = Barratta, aes(x=Daily.Ave.PAF, fill=Season)) + # this line tells r which dataset to use (data =...) and which column to plot (x =...) 
...
z <- ggplot(Barratta, aes(x = Month, y = Daily.Ave.PAF, fill = Season)) +
...
fit.norm <- fitdist(Barratta$logVALUE, "norm")
plot(fit.norm)

fit.norm <- fitdist(Barratta$sqrtVALUE, "norm")
plot(fit.norm)
...
Dist_check_Barratta <- descdist(Barratta$sqrtVALUE, discrete = FALSE, boot = 1000)
Dist_check_Barratta <- descdist(Barratta$logVALUE, discrete = FALSE, boot = 1000)
...
fit = mcp(model, cores = 3, data = Barratta)
...
write.csv(fit_summary, "Barratta fit summary_JAGS.csv", row.names = FALSE)
...
# extract cp and find closest date for plotting
{
  ...
  print(cp_1) # view
  differences <- abs(Barratta$time - cp_1) # calculate closest sqrtVALUE to cp_1
  closest_index <- which.min(differences) # find the index of this value 
  closest_date <- Barratta$DATE[closest_index] # Get the corresponding DATE value that aligns with this index
...
BarrattaPlot <- plot(fit, q_predict = TRUE) + ggtitle("Change Point of Total Pesticides in Barratta Creek at Northcote") + 
  labs(y= "sqrt time-series PAF (%)", x = "Days since first sample
(01/08/2011 - 25/06/2023)") + 
  #geom_vline(xintercept=cp_1, linetype='solid', col='red') +
  #annotate("text", x=2800, y=9, label=closest_date, size=4, color="dark blue", angle = 90) +
...
BarrattaPlot
ggsave("Barratta CP with seasonality_22000.png", width = 16, height = 9, dpi = 1000)
